...
def deep_len(lnk):
    ...
    if lnk == Link.empty:
        return 0
    if not isinstance(lnk.first, int):
...
def make_to_string(front, mid, back, empty_repr):
    ...
    def help(lnk):
        if lnk == Link.empty:
            ...
            return front + str(lnk.first) + mid + help(lnk.rest) + back
    return help
...
def tree_map(fn, t):
    ...
    if t.is_leaf():
        ...
        bran = [tree_map(fn, b) for b in t.branches]
        return Tree(fn(t.label), bran)
...
def long_paths(tree, n):
    ...
    for b in tree.branches:
        for x in long_paths(b, n-1):
            paths.append(Link(tree.label, x))
    return paths
...
def subseqs(s):
    ...
    if not s:
        ...
        x = subseqs(s[1:])
        return insert_into_all(s[0], x) + x
...
def trade(first, second):
    ...
    e = lambda: sum(first[:m]) == sum(second[:n])
    while m < len(first) and n < len(second) and not e():
        if sum(first[:m]) < sum(second[:n]):
            m += 1
        else:
            n += 1

    if e(): # change this line!
