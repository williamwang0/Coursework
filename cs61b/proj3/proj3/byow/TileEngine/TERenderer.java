...
public class TERenderer {
    ...
    public void initialize(int w, int h, int xOff, int yOff) {
        ...
        StdDraw.text(width / 2, height * 5 / 8, "Denero's Dungeon");
        font = new Font("Monaco", Font.BOLD, TILE_SIZE);
        StdDraw.setFont(font);
        StdDraw.text(width / 2, height / 2, "[N]ew Game");
        StdDraw.text(width / 2, height / 2 - 1.2, "[L]oad Save");
        StdDraw.text(width / 2, height / 2 - 2.4, "[R]eplay Save");
        StdDraw.text(width / 2, height / 2 - 3.6, "[Q]uit");
    ...
    public void renderFrame(TETile[][] world) {
        ...
        if (mouseX < numXTiles && mouseY < numYTiles && mouseX >= 0 && mouseY >= 0) {
            tileDescription = "Wall Description: " + world[mouseX][mouseY].description();
        }
        StdDraw.setPenColor(Color.WHITE);
        StdDraw.text(numXTiles / 2, 1, tileDescription);
    ...
    public void seedScreen() {
        StdDraw.clear(new Color(0, 0, 0));
        StdDraw.setPenColor(Color.WHITE);
        StdDraw.text(width / 2, height / 2 + 0.6, "please enter a world generation seed");
        StdDraw.text(width / 2, height / 2 - 0.6, "press [S] when done");
        StdDraw.show();
    }

    public void loading() {
        StdDraw.setPenColor(Color.WHITE);
        StdDraw.text(width / 2, height / 4, "loading save...");
        StdDraw.show();
    }
}
