    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                if insect.can_contain(self.ant) and insect.contained_ant == None:
                    insect.contain_ant(self.ant)
                    self.ant = insect
                elif self.ant.can_contain(insect) and self.ant.contained_ant == None:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        count = 0
        positionCheck = self.place
        while positionCheck != hive:
            if positionCheck.bees != []:
                if count >= self.min_range and count <= self.max_range:
                    return random_or_none(positionCheck.bees)
            positionCheck = positionCheck.entrance
            count += 1
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount = 3):
        ...
        if self.armor <= 0:
            while index < len(self.place.bees):
                if self.place.bees[index].armor <= self.damage:
                    self.place.remove_insect(self.place.bees[index])
                else:
                    self.place.bees[index].armor -= self.damage
                    index += 1
            self.place.remove_insect(self)
        # END Problem 5
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        self.armor = armor
        self.queen_count += [1]
        if len(self.queen_count) > 1:
            self.queen_type = 'impostor'
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 13
        if self.queen_type == 'impostor':
            Insect.reduce_armor(self, self.armor)
        else:
            location = self.place.exit
            while location != None:
                if location.ant != None:
                    if location.ant not in self.ants_empowered:
                        location.ant.damage *= 2
                        self.ants_empowered += [location.ant]    
                    if location.ant.is_container and location.ant.contained_ant != None:
                        if location.ant.contained_ant not in self.ants_empowered:    
                            location.ant.contained_ant.damage *= 2
                            self.ants_empowered += [location.ant.contained_ant]
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 13
        self.armor -= amount
        if self.armor <= 0 and self.queen_type == 'true':
            bees_win()
        # END Problem 13
